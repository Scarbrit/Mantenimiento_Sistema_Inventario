...
// Optimización N+1: Obtener productos con variantes en una sola consulta
export const getAllProductsWithVariants = async (filters = {}) => {
  ...
  if (filters.category_id) {
    baseConditions += ` AND p.category_id = $${paramCount}`;
    params.push(filters.category_id);
    paramCount++;
  }

  if (filters.brand_id) {
    baseConditions += ` AND p.brand_id = $${paramCount}`;
    params.push(filters.brand_id);
    paramCount++;
  }

  if (filters.search) {
    baseConditions += ` AND (p.name ILIKE $${paramCount} OR p.sku ILIKE $${paramCount} OR p.description ILIKE $${paramCount})`;
    params.push(`%${filters.search}%`);
    paramCount++;
  }
  ...
  const query = `
    ...
    ${baseConditions}
    GROUP BY p.id, c.name, b.name, u.first_name, u.last_name
